...
def get_installed_extensions():
    ...
    # NOTE: Upstream prefers the `importlib_metadata` backport on Python < 3.10,
    # but that backport is not vendored here and is unavailable in Sublime Text's
    # embedded Python 3.8 runtime. The stdlib `importlib.metadata` has shipped
    # since Python 3.8, so we use it directly on all versions.
    from importlib import metadata
    # Only load extension entry_points once.
    eps = metadata.entry_points()
    if hasattr(eps, 'select'):
        return eps.select(group='markdown.extensions')
    return eps.get('markdown.extensions', [])
