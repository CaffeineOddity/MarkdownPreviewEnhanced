...
#
# NOTE: The upstream code uses ``importlib.util.find_spec`` + ``exec_module`` to
# load a *fresh copy* of ``html.parser``. That fails inside Sublime Text, whose
# Python runtime serves parts of the stdlib from a zip archive: the resolved
# loader is a ``zipimporter``, which has no ``exec_module`` method, raising
# ``AttributeError: 'zipimporter' object has no attribute 'exec_module'``.
# Instead we import the real ``html.parser`` module and create a standalone copy
# of the ``HTMLParser`` class (and the module-level regexes we need to patch) so
# our monkeypatches do not bleed onto the shared stdlib module.
import types as _types
from html import parser as _stdlib_html_parser

htmlparser = _types.ModuleType("htmlparser")
htmlparser.HTMLParser = _stdlib_html_parser.HTMLParser
# Copy the module-level attributes the code below (and HTMLParser internals)
# reference, so patching them here does not mutate the real stdlib module.
for _attr in (
    "piclose", "entityref", "incomplete", "locatestarttagend_tolerant",
    "tagfind_tolerant", "attrfind_tolerant", "endendtag", "endtagfind",
    "commentclose", "declname", "cdataelem", "unescape",
):
    if hasattr(_stdlib_html_parser, _attr):
        setattr(htmlparser, _attr, getattr(_stdlib_html_parser, _attr))
